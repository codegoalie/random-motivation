...
REMOTE_HOST="casaos"
COMPOSE_FILE="/var/lib/casaos/apps/kind-eldan/docker-compose.yml"

# Update the image in docker-compose.yml on remote host
# shellcheck disable=SC2029
ssh "${REMOTE_HOST}" "sed -i 's|image: ${IMAGE_NAME}:.*|image: ${IMAGE_TAG}|g' ${COMPOSE_FILE}"

echo "🔄 Restarting application on remote host"
# Navigate to the directory and restart using docker compose
ssh "${REMOTE_HOST}" "cd /var/lib/casaos/apps/kind-eldan && docker compose down && docker compose up -d"
